...
7. **Reference section IDs** - Every observation must specify which `data-section` it applies to (hero, features, pricing, etc.). This enables the review overlay to highlight the exact section when hovering over findings.

## Section-Aware Output (Critical for Review System)

When generating persona observations, each finding MUST include a `category` (typography, color, layout, etc.) AND map to a specific page section from the design's section map.

The section map is defined by `/ui-planner` and enforced by `/create-task`. Findings that don't reference a valid section cannot be displayed in the review overlay.

**Pattern:**
```typescript
{
  category: 'TYPOGRAPHY',       // What aspect
  section: 'hero',              // Which page section (data-section value)
  observation: 'The headline is too small on mobile',
  verdict: 'needs-work',
  comment: 'I can barely read it on my phone...'
}
```

**Valid sections** depend on the project's design. Common examples:
- `hero`, `features`, `pricing`, `testimonials`, `faq`, `signup` (landing page)
- `header`, `metrics`, `charts`, `filters`, `table` (dashboard)
- `hero`, `products`, `cart`, `checkout` (e-commerce)
