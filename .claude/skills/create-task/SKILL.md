---
name: create-task
description: Plan and implement tasks for the SLAP WebUI with e2e test validation. Use when the user asks to implement a feature, fix a bug, update functionality, or any task that modifies the WebUI codebase.
...
You are implementing tasks for the SLAP WebUI. Each task includes planning, implementation, and **automated e2e test validation**.
...
| 9 | **Section attributes** | Every top-level page section gets `data-section` for review highlighting |
| 10 | **State exposure** | Expose review state via `window.slapState` |

## IMPORTANT: Section Discoverability

Every design component MUST follow the dual-attribute pattern:

```tsx
// data-section = review region (for SectionHighlight overlay)
// data-testid = element identity (for e2e testing)
<section data-section="hero" data-testid="hero-section">
  <h1 data-testid="hero-headline">...</h1>
  <button data-testid="hero-cta-primary">...</button>
</section>
```

**Rules:**
1. Every top-level page region gets `data-section="{sectionId}"` — this is what the review overlay targets
2. `data-section` values MUST match the section keys in review findings (`sections: Record<string, Finding[]>`)
3. Section IDs are flat — "hero" not "page-hero-left"
4. Interactive elements within sections get `data-testid` following `{feature}-{element}` pattern
5. `data-section` and `data-testid` serve different purposes — both are required on section wrappers

**State exposure:**
```typescript
// Expose on every project page for review system + e2e
window.slapState = {
  project: string,
  version: string,
...
3. **Section map** - Which `data-section` attributes this component needs (must match review finding keys)
4. **New testids** - Any `data-testid` attributes to add (semantic: `{feature}-{element}-{action}`)
5. **E2E test cases** - What to verify automatically
6. **State exposure** - What to add to `window.slapState`
