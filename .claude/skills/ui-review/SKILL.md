...
After aesthetic and styleguide auditing, validate that the design's section map is properly implemented for the review system.

### Section Map Check

1. **Read the section map** from `/ui-planner` output or `design-contract`
2. **Scan the preview HTML** for `data-section` attributes
3. **Cross-reference** sections with review findings

| Check | Pass | Fail |
|-------|------|------|
| Every section in map has `data-section` in DOM | All found | Missing sections listed |
| Every `data-section` in DOM is in the map | All mapped | Orphan sections listed |
| Every section has `data-testid` alongside `data-section` | Both present | Missing testids listed |
| Review findings reference valid section IDs | All match | Mismatched keys listed |
...
| Section ID | In DOM? | In Findings? | data-testid? | Status |
|------------|---------|--------------|-------------|--------|
| hero | YES | YES (5 experts, 12 personas) | hero-section | OK |
| features | YES | YES (5 experts, 10 personas) | features-section | OK |
| pricing | YES | NO | pricing-section | UNCOVERED |
| signup | NO | YES (3 experts) | — | MISSING DOM |
```

**Blocking issues:**
- Section in findings but NOT in DOM → review overlay can't highlight it
- Section in DOM but NOT in findings → gap in review coverage
