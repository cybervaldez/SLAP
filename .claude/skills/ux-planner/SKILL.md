...
### Page Sections (for review system)
Identify the top-level sections this feature/page will have. These become `data-section` attributes and review finding keys.

| Section ID | Purpose | Key Interactions |
|------------|---------|-----------------|
| {id} | {what this section does} | {user actions in this section} |

Example for a landing page:
| Section ID | Purpose | Key Interactions |
|------------|---------|-----------------|
| hero | First impression, value prop | Read headline, click CTA |
| features | Feature showcase | Browse cards, hover for details |
| pricing | Pricing tiers | Compare tiers, click signup |
| testimonials | Social proof | Read quotes, see logos |
| faq | Common questions | Expand/collapse items |
| signup | Final conversion | Fill form, submit |

**Why this matters:** These section IDs flow through the entire pipeline:
- `/ui-planner` maps them to visual regions
- `/create-task` implements them as `data-section` attributes
- `/kaizen` + `/team` generate findings keyed to these sections
- The review overlay highlights these sections when hovering findings
