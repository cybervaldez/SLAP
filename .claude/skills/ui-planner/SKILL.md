...
### Section Map (for review highlighting)

Every design page is divided into reviewable sections. Each section gets a `data-section` attribute that the review overlay uses for highlighting. This map is the contract between the design and the review system — findings reference these exact IDs.

| Section ID | Element | Purpose | data-section |
|------------|---------|---------|-------------|
| {id} | {element description} | {what it contains} | `data-section="{id}"` |

Example for landing page:
| Section ID | Element | Purpose | data-section |
|------------|---------|---------|-------------|
| hero | Hero section | Headline, CTA, hero image | `data-section="hero"` |
| features | Features grid | Feature cards | `data-section="features"` |
| pricing | Pricing table | Tiers, comparison | `data-section="pricing"` |
| testimonials | Social proof | Customer quotes | `data-section="testimonials"` |
| faq | FAQ accordion | Questions and answers | `data-section="faq"` |
| signup | CTA/signup | Final conversion | `data-section="signup"` |

**Rules:**
- Section IDs are flat (no nesting: "hero" not "page > hero > left-column")
- Every top-level page region gets a `data-section`
- Section IDs match the keys used in review findings (`sections: Record<string, Finding[]>`)
- `data-section` is for review highlighting; `data-testid` is for e2e testing (both required)
...
### Layout Regions (testable + reviewable)
Each region has BOTH attributes:
- `data-section="hero" data-testid="hero-section"` - {description}
- `data-section="features" data-testid="features-section"` - {description}
- `data-section="pricing" data-testid="pricing-section"` - {description}

### Discoverability Requirements
| Requirement | Attribute | Purpose |
|-------------|-----------|---------|
| Review sections | `data-section="{sectionId}"` | SectionHighlight glow targets |
| E2E element IDs | `data-testid="{feature}-{element}"` | Automated test selectors |
| Theme state | `data-theme="light\|dark"` on root | Theme toggle verification |
| App state | `window.slapState` object | Review system state exposure |
